...
angular.module('myApp',[])
	.controller("CityController",function($scope,$http){
		...
		var remoteIP = "http://"+sessionStorage.addr+"/faceattribute";
		...
		$scope.$watch('$viewContentLoaded', function() {
			...
			if (img.complete || img.complete === undefined) { // 确保对缓存的图片也触发img.onload事件
				img.src = "pic/faceattribute/1.jpeg";
		...
		$scope.getFaceAttribute = function(message){
			$http({
	                ...
	                if(data.result==0)
	                {
						...
						d=parseInt(data.beauty)
	                     $scope.attributes = {"性别":a,"年龄":b,"种族":c,"颜值":d}						
		...
		// 画框框函数
		$scope.drawFrame = function(whichFace){
			//获取图像XY起点坐标和percent
			...
			var width = 60;//faceMsg.width/percent;
			var height =60;// faceMsg.height/percent;
		...
		$scope.authByFeature = function(){
			if($scope.left.detection ){
				if($scope.left.faceInfo.feature ){
					var data = {
						'feature': $scope.left.faceInfo.feature
