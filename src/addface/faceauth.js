angular.module('myApp',[])
	.controller("CityController",function($scope,$http){
		var remoteIP = "http://"+sessionStorage.addr+"/addface";
		...
		$scope.nextimg=function(){
			...
			function handleClick (e) {			
				...
				img.onload = function() {
					...
					$scope.imageDataBase64 = aa.substr(aa.indexOf('base64,') + 'base64,'.length);
					//console.log($scope.imageData)
					$scope.message  = {"face":$scope.imageDataBase64,"uid":$scope.uid,"user_id":"gzntdemo"};
		...
		$scope.$watch('$viewContentLoaded', function() {
			...
			img.onload = function() {
				...
				function b() {
					document.getElementById("myCanvas-left").style.opacity=1;
					//document.getElementById("myCanvas-left").classList.add('blur');
					document.getElementById("can").style.display="none";	
		...
		$scope.img_upload = function(msg,files) {       //单次提交图片的函数
			...
			$scope.reader.onload = function(ev) {
					...
					image.onload = function(){
						...
						document.getElementById('result').innerHTML = '';
						//绘图
						context.drawImage(image, imageStartPointX+3, imageStartPointY+3, drawObj.width-6, drawObj.height-6); //绘制图像，drawImage有多种重载函数，具体参考w3l，至此载入图片完毕
					  //绘制边框
				      // context.strokeStyle = "#379edf";
				      // context.lineWidth = 3;
			        ...
			        //$scope.addface(message);
				};
		};

		$scope.register = function(msg,files) { 
			if($scope.uid){
			}else{
				$.alertable.alert('请输入用户ID')
				return
			}

			if($scope.imageDataBase64){
			}else{
				$.alertable.alert('请上传图片');
				return;
			}

			$scope.message["uid"] = $scope.uid;
			$scope.addface($scope.message);
		}

		$scope.addface = function(message){
			$http({
	                method: 'post',
	                url: remoteIP,
	                data:message,
	                ...
	                if(data.result==0)
	                {
						//$scope.jsondata = $scope.format(angular.toJson(data));
						document.getElementById('result').innerHTML = $scope.format(angular.toJson(data),'')
	                	//清除图片
	                	    var c=document.getElementById("myCanvas-left");  
	                	...
	                	//清除值
	                	$scope.uid=undefined;
	                	$scope.imageDataBase64=undefined;
	                    console.log("receive from service");
						$.alertable.alert('注册成功')
	                }
	                else{
						$.alertable.alert("人脸注册失败: " + data.message);
	                }
