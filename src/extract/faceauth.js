angular.module('myApp',[])
	.controller("CityController",function($scope,$http){
		var remoteIP = "http://"+sessionStorage.addr+"/extract";
			...
			function handleClick (e) {			
				...
				img.onload = function() {
					...
					function a(callback) {
						...
						document.getElementById("feature___").innerHTML = '';
					...
					function b() {
						...
						var message = {"face":$scope.imageData,"user_id":"gzntdemo"};
	
						$scope.getFaceAttribute(message);
					}
		...
		$scope.$watch('$viewContentLoaded', function() {
			...
			if (img.complete || img.complete === undefined) { // 确保对缓存的图片也触发img.onload事件
				img.src = "http://i2.bvimg.com/659442/8dd7d752be037291.jpg";
		...
		$scope.img_upload = function(msg,files) {       //单次提交图片的函数
			...
			$scope.reader.onload = function(ev) {
					...
					var message = {"face":$scope.imageDataBase64,"user_id":"gzntdemo"};

			        $scope.getFaceAttribute(message);
				};
		};

		$scope.getFaceAttribute = function(message){
			$http({
	                method: 'post',
	                url: remoteIP,
	                data:message,
	                headers: {'Content-Type': "application/json"}
								 ...
								 var feature___ = document.getElementById("feature___");
								var feature__ = document.getElementById("feature__");
	                if(data.result==0)
	                {
						console.log("receive from service");
						feature___.innerHTML =  data.feature//.substring(0,300) +"......";
						//feature__.value = '';
						 feature__.value = data.feature;
						//feature__.style.display='none';

	                }
	                else{
	                    $.alertable.alert("提取失败: " + data.message);
					}
